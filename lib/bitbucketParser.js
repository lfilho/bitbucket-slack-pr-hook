    ...
    var messageHandlers = {
        opened: function (pr) {
            ...
            if (util.FEATURE_SWITCH.mentionReviewers && data.reviewers && data.reviewers.length > 0) {
                var reviewersStr = '';
                data.reviewers.forEach(function(reviewer) { reviewersStr += ' <@' + reviewer.user.displayName + '>'; });
                result.fields.push({
                    title: 'Reviewers:',
        ...
        cadded: function (pr) {
            ...
            return {
                fallback: data.user + ' *posted* <' +  util.commentUrl(data.prUrl[0].href, data.commentId) + '|a comment> on a ' + '<' + data.prUrl[0].href + '|PR.>:',
        ...
        cdeleted: function (pr) {
            ...
            return {
                fallback: data.user + ' *deleted* <' + util.commentUrl(data.prUrl[0].href, data.commentId) + '|a comment> on a ' + '<' + data.prUrl[0].href + '|PR.>:',
                color: util.COLORS.yellow,
                fields: [{
                    value: util.truncate(data.commentText)
                }]
        ...
        cedited: function (pr) {
            ...
            return {
                fallback: data.user + ' *updated* <' + util.commentUrl(data.prUrl[0].href, data.commentId) + '|a comment> on a ' + '<' + data.prUrl[0].href + '|PR.>:',
                color: util.COLORS.yellow,
                fields: [{
                    value: util.truncate(data.commentText)
                }]
